	<div class="us-table-wrapper order_table" v-loading="loading">
				...
				<table class="us-table">
					<thead>
						<tr>
							...
							<th>送货日期</th>
							<th>状态</th>
							<th v-if="!isHistoryOrderPage">操作</th>
						</tr>
					</thead>
					<tbody>
						<tr v-for="(item, i) of tableDatas" :key="i">
							<td>
								<!-- 物流单号，点击跳转到详情页 -->
								<el-badge is-dot class="item us-cell-blue" :hidden="isHistoryOrderPage?true:item.carrierOrderUnread"
									@click.native="jumpDetailPage(item.logisticsNumber)">
							...
							<td style="position: relative">
								<!-- 名称 -->
								<template v-if="item.clientShortName">
									<div class="client-name-text">
										{{ item.clientShortName }}
									</div>
									<!-- 客户级别图标 -->
									<img v-if="item.clientLevel !== null" class="client-level-icon" :src="getLevelIcon(item.clientLevel)" />
								</template>
							</td>
							<!-- 送货地址 -->
							<td>{{ item.deliveryAddressAreaNames.split(',')[0] }}</td>
							<!-- 提货日期 -->
							<td>{{ item.takingDate | formatDate }}</td>
							<!-- 送货日期 -->
							<td>{{ item.deliveryDate | formatDate }}</td>
							<!-- 状态 -->
							<td :class="carrierOrderStatusClass[item.carrierOrderStatus]">
								{{ carrierOrderStatusNameMap[item.carrierOrderStatus] }}
							</td>
							<!-- 操作列 -->
							<td v-if="!isHistoryOrderPage">
								<el-button v-if="isUnrelation"  type="text" @click="jumpEditPage(item.logisticsNumber)">查看</el-button>
								<el-button v-if="!isUnrelation" type="text" @click="jumpEditPage(item.logisticsNumber)">编辑</el-button>
								<el-button v-if="isUnassignOrder(item.carrierOrderStatus)" type="text"
										@click="unassignConfirmHandle(item)">取消分配</el-button>
							</td>
						</tr>
					</tbody>
				</table>
    ...
    <div class="StayaboutCarEdit edit-stayboutcar">
        ...
        <div style="margin-top: 40px;overflow: hidden">
            ...
            <div style="margin-top: 28px" v-loading="loading">
                <el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="110px" class="demo-form-inline"
                         label-position="left">
                         <el-row>
                             <el-col :span="8">
                            ...
                            <div class="Deliverydistance" style="clear: both" v-for="item of ruleForm.carAndDriverList">
                                <el-form-item :label="item.plateNumber">
                                    <div class="allcareimg" v-show="item.shipmentDriverOrderFileVoList.length>0">
                                        <viewer>
                                            <div class="carimg"
                                                 v-for="(value,index) of item.shipmentDriverOrderFileVoList">
                                                <div class="singlimg">
                                                    <img :src="value.url"
                        ...
                        <template v-if="item.unusualDriverOrderFileVoList.length">
                            ...
                            <div style="margin-top: 28px" v-loading="loading">
                                <div class="Deliverydistance;" style="clear: both;margin-top: 30px"
                                    v-for="value of item.unusualDriverOrderFileVoList" :key="value">
                                    <el-form-item :label="item.plateNumber">
                        ...
                        <div style="margin-top: 28px" v-loading="loading">
                            ...
                            <div v-if="orderKind === 'FC'">
                                <el-row>
                                    ...
                                    <el-col :span="6" v-show="ruleForm.isWarehouse === 1" >
                                        <el-form-item prop="warehouseId">
                                            <el-select v-model="ruleForm.warehouseId" :disabled="isDisable === 1 ? true : false" placeholder="请选择仓库">
                                              <el-option
        ...
        <div class="change-btn-wrapper">
            ...
            <el-button :disabled="orderKind === 'OF'"  v-show="ruleForm.orderStatus!=='5'" style="width:13.5%;padding: 0px 40px"
